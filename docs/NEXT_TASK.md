...
| T-017 | Deploy RPi4 Node 1 | Guida completa (`docs/deployment/DEPLOYMENT.md`) + bootstrap automatico (`scripts/deploy/bootstrap_rpi4.sh`): Ubuntu 24.04 arm64, Docker, segreti+certificati, stack up, verifica, troubleshooting | Alta | — | 2g | ✅ Fatto 2026-08-25 (doc+script; esecuzione fisica sul Pi da validare) |
| T-018 | Validare deploy su hardware reale | Eseguire il bootstrap su un RPi4 8GB reale: primo boot SSD/SD, stack healthy dopo reboot, certificati letti da mosquitto, memoria entro budget (~4GB) | Alta | T-017 | 0.5g | ⬜ Da fare (richiede hardware) |
